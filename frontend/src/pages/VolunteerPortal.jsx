...
import { fetchVolunteers, fetchIncidents, confirmAssignment, geoCheckin, completeTask, volunteerAPI } from '../api';
...
// ── Directory card — privacy-safe, read-only tile for community view ─
function DirectoryCard({ vol }) {
  // Extract a readable city/state from the end of the address string.
  // e.g. "12 Relief Camp Road, Jaipur, Rajasthan" → "Jaipur, Rajasthan"
  const cityState = vol.address
    ? vol.address.split(',').slice(-2).join(',').trim()
    : null;

  return (
    <div className="vol-roster-card">
      <div className="vol-roster-card-top">
        <div className="vol-roster-avatar">{vol.name[0]}</div>
      </div>
      <div className="vol-roster-name">{vol.name}</div>
      {vol.domain_knowledge && (
        <div className="vol-roster-skills">{vol.domain_knowledge}</div>
      )}
      {cityState && (
        <div className="vol-roster-skills">
          <MapPin size={10} style={{ display: 'inline', marginRight: '3px', verticalAlign: 'middle' }} />
          {cityState}
        </div>
      )}
    </div>
...
function RosterView({ volunteers, onSelect, directory, dirLoading }) {
  ...
  return (
    <div className="vol-portal">
      ...
      <div className="vol-roster">
        ...
        {/* Community Directory — privacy-safe view from User registry */}
        <section className="vol-roster-section">
          <h3 className="vol-roster-section-title">
            <BookOpen size={14} strokeWidth={2.2} />
            Community Directory
            {!dirLoading && (
              <span className="vol-section-count">{directory.length}</span>
            )}
          </h3>
          {dirLoading ? (
            <div className="vol-roster-empty">
              <Loader size={16} className="field-spin" />
              <span>Loading directory…</span>
            </div>
          ) : directory.length === 0 ? (
            <div className="vol-roster-empty">No registered volunteers found</div>
          ) : (
            <div className="vol-roster-grid">
              {directory.map((v, i) => (
                <DirectoryCard key={`${v.name}-${i}`} vol={v} />
              ))}
            </div>
          )}
        </section>
...
export default function VolunteerPortal() {
  ...
  // Fetch the privacy-safe community directory once on mount (or when token changes).
  // This hits a protected endpoint — email, phone, and GPS coords are never returned.
  useEffect(() => {
    if (!token) { setDirLoading(false); return; }
    volunteerAPI.getDirectory(token)
      .then((data) => setDirectory(data.volunteers || []))
      .catch((err) => console.error('[directory] fetch failed:', err.message))
